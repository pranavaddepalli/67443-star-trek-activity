...
let a = humanOfficers.count
print(a)


// Q1: What percentage of humans are female?
let b = Float(humanOfficers.filter{ femaleOfficers.contains($0)}.count) / Float(humanOfficers.count)
print(b)


// Q2: How many first officers are human?
let c = firstOfficers.keys.filter{humanOfficers.contains($0)}.count
print(c)


// Q3: Get a list of females sorted by name
  let d = femaleOfficers.sorted(by: {$0 < $1})
print(d)
...
let firstVulcansShorter5 = firstVulcans.filter{$0.name.count < 5}
print(firstVulcansShorter5.map{$0.name})
